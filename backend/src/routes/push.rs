...
pub async fn subscribe(
    ...
    push::register(&state.pool, body.map_err(|_| ApiError::InvalidInput)?.0)
        .await
        .map(Json)
}
pub async fn unsubscribe(
    ...
    push::unregister(&state.pool, &id).await?;
...
pub async fn presence(
    ...
    push::presence(&state.pool, body.map_err(|_| ApiError::InvalidInput)?.0)
        .await
        .map(Json)
}
