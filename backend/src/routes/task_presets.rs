use crate::{
    AppState,
    errors::ApiError,
    services::{ListQuery, Result, task_presets as service},
};
use axum::{
    Json,
    extract::{
...
pub async fn groups(State(state): State<AppState>) -> Result<Json<Value>> {
    service::groups(&state.pool).await.map(Json)
}
