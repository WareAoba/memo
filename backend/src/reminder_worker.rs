...
pub async fn claim(pool: &SqlitePool) -> Result<Vec<Job>, sqlx::Error> {
    ...
    // Resume unsent cancellations when the same reminder becomes eligible again.
    // Preserve attempts and backoff; sent/failed deliveries must never be revived.
    sqlx::query("INSERT INTO push_deliveries(id,subscription_id,schedule_id,reminder_version) SELECT lower(hex(randomblob(16))),p.id,s.id,s.reminder_version FROM schedules s JOIN push_subscriptions p ON p.user_id=s.user_id AND p.enabled=1 AND EXISTS(SELECT 1 FROM users u WHERE u.id=p.user_id AND COALESCE(json_extract(u.settings_json,'$.push_enabled'),1)=1) WHERE s.reminder_enabled=1 AND s.status IN ('planned','in_progress') AND s.reminder_at<=unixepoch() AND s.reminder_start_at>unixepoch() ON CONFLICT(subscription_id,schedule_id,reminder_version) DO UPDATE SET status='pending',lease_token=NULL,lease_until=NULL,updated_at=unixepoch() WHERE push_deliveries.status='cancelled' AND push_deliveries.attempts<5")
