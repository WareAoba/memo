-- Preserve schedules and all dependent snapshots while aligning schedule states.
...
CREATE TABLE schedule_entity_snapshot_v3 (
 id TEXT PRIMARY KEY NOT NULL, schedule_id TEXT NOT NULL UNIQUE REFERENCES schedules_v3(id),
 definition TEXT NOT NULL CHECK(json_valid(definition)),
 created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),
 updated_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now'))
);
CREATE TABLE schedule_tasks_v3 (
 ...
 created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),
 updated_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),
 UNIQUE(schedule_id,position), FOREIGN KEY(schedule_id,user_id) REFERENCES schedules_v3(id,user_id),
 FOREIGN KEY(source_task_preset_id,user_id) REFERENCES task_presets(id,user_id)
);

CREATE TABLE schedule_task_items_v3 (
 id TEXT PRIMARY KEY NOT NULL, schedule_task_id TEXT NOT NULL REFERENCES schedule_tasks_v3(id),
...
INSERT INTO schedules_v3(id,user_id,entity_id,title,scheduled_date,end_date,start_time,end_time,time_zone,notes,status,deleted_at,created_at,updated_at)
 SELECT id,user_id,entity_id,title,scheduled_date,end_date,start_time,end_time,time_zone,notes,CASE status WHEN 'pending' THEN 'planned' WHEN 'skipped' THEN 'cancelled' ELSE status END,deleted_at,created_at,updated_at FROM schedules;
INSERT INTO schedule_entity_snapshot_v3 SELECT * FROM schedule_entity_snapshot;
INSERT INTO schedule_tasks_v3 SELECT * FROM schedule_tasks;
INSERT INTO schedule_task_items_v3 SELECT * FROM schedule_task_items;
...
ALTER TABLE schedule_entity_snapshot_v3 RENAME TO schedule_entity_snapshot;
ALTER TABLE schedule_tasks_v3 RENAME TO schedule_tasks;
ALTER TABLE schedule_task_items_v3 RENAME TO schedule_task_items;
