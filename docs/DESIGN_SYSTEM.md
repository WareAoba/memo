...
새 색상은 의미를 가진 토큰으로 정의한다. 캔버스/표면/보조 표면, 본문/보조 글자, 강조/선택, 완료/주의/오류를 구분한다. 시계의 시간대 그라데이션과 일정 상태 색상도 토큰에서 관리한다. 다크 모드의 시간 종료·미완료 호는 흰색이다. 연속 일정은 현재 호 두께를 최소로 5단계 외곽 높이를 사용하며 ID 기반 값과 인접 중복 방지로 매 렌더의 흔들림을 막는다. 완료 비율에 따른 연속 HSL 계산은 `scheduleAppearance.ts`, 연간 달성률 계산은 기존 캘린더 로직에 유지한다.
